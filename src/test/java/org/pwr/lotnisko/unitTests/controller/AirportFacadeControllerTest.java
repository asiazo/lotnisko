...
class AirportFacadeControllerTest {
    ...
    //             .date(new Date())
    //             .build();
    //     when(reservationService.addReservation(ArgumentMatchers.any(TicketTO.class)))
    //             .thenReturn(reservation);
    //
    //     // when
    //     ResultActions result = mvc.perform(post("/api/v1/zakupbiletu")
    //             .contentType(MediaType.APPLICATION_JSON)
    //             .content(JsonUtils.getJson(TicketTO.builder().flightId(1L).discountType(DiscountType.STUDENT)
    //                             .flightId(1L).build())));
    //
    //     // then
